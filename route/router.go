...
type Router struct {
	...
	BittorrentOutbound 				   adapter.Outbound
...
func NewRouter(
	...
	router.directusermap = &sync.Map{}

	for i, ruleOptions := range options.Rules {
		
		if ruleOptions.Type == "botrule" {
			router.rules = append(router.rules, NewBotRule(ruleOptions.DefaultOptions.Outbound))
...
func (r *Router) Initialize(inbounds []adapter.Inbound, outbounds []adapter.Outbound, defaultOutbound func() adapter.Outbound) error {
	...
	for _, detour := range outbounds {
		...
		if detour.Type() == "block" {
			r.BittorrentOutbound = detour 
		}
		
		if detour.Type() == "torrent" {
			r.BittorrentOutbound = detour
			break
		}
...
func (r *Router) RouteConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext) error {
	...
	if metadata.InboundOptions.SniffEnabled && !sniff.Skip(metadata) && r.SkipSniff(metadata.User) {
	...
	if metadata.Protocol == C.ProtocolBitTorrent { //alredy skipped other detoures 
		detour = r.BittorrentOutbound
	}
